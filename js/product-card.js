...
export function renderProductCard (p, shopName, rating = null, wunschliste = false) {
  ...
  const preisHtml = sale
    ? (() => {
        ...
        return `<span class="product-card__price-alt">${esc(preis)}</span><span class="product-card__price product-card__price--sale">${euro.format(p.angebotspreis)}</span><span class="product-card__badge product-card__badge--ersparnis">−${ersparnis}</span>`
      })()
