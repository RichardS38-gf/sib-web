...
function renderDetail (produkt, alleVarianten = [], farben = []) {
  ...
  // Geschlecht -- nur anzeigen, wenn gesetzt (Bestandsprodukte ohne Wert weglassen)
...
// ── Details-Sektion (Beschreibung + Features + Bild(er)) ──
function renderDetails (produkt) {
  ...
  const bilder = (Array.isArray(produkt.details_bilder) && produkt.details_bilder.length)
  ...
  if (bilder.length === 1) {
    ...
    // Bewegte Collage statt Slider: die Fotos laufen als lockerer, leicht
    // schiefer Stapel durch und rechts über den Bildschirmrand hinaus.
    // Fuer eine nahtlose Endlosschleife wird die Bilderreihe verdoppelt.
    const rotationen = [-4, 3, -2, 5, -3, 2]
    ...
    bildHtml = `
      <div class="pdp-details__collage-bleed">
        <div class="pdp-details__collage-track">
          ${kartenHtml(0)}
          ${kartenHtml(1)}
        </div>
      </div>`
