...
// Zeigt Chat-Bubble wenn Sessions in localStorage existieren.
// Mehrere Chats → Reiter zum Wechseln.
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith('sib_chat_')) {
      try {
        ...
        if (s?.chat_id) sessions.push({ ...s, storageKey: key })
      } catch {}
    }
  }
  return sessions
...
function injectStyles () {
  ...
  s.textContent = `
    ...
    .gcw-trigger:hover{transform:scale(1.08)}
    ...
    .gcw-panel{position:fixed;bottom:5rem;right:1.5rem;width:360px;max-height:540px;background:#fff;border-radius:16px;box-shadow:0 8px 40px rgba(0,0,0,.18);display:flex;flex-direction:column;overflow:hidden;animation:gcwSlide .2s ease;z-index:900}
    ...
    .gcw-close{background:none;border:none;color:rgba(255,255,255,.7);cursor:pointer;padding:.25rem;border-radius:4px;display:flex;transition:color .15s;flex-shrink:0}
    .gcw-close:hover{color:#fff}
    .gcw-tabs{display:flex;border-bottom:1px solid #e5e5e5;overflow-x:auto;flex-shrink:0;scrollbar-width:none}
    .gcw-tabs::-webkit-scrollbar{display:none}
    .gcw-tab{padding:.5rem .9rem;font-size:.75rem;font-weight:600;border:none;border-bottom:2px solid transparent;background:none;cursor:pointer;color:#888;white-space:nowrap;transition:color .15s,border-color .15s;display:flex;align-items:center;gap:.35rem}
    .gcw-tab img{width:16px;height:16px;border-radius:3px;object-fit:cover}
    .gcw-tab.active{color:#0D0D0D;border-bottom-color:#0D0D0D}
    .gcw-tab:hover{color:#333}
    .gcw-messages{flex:1;overflow-y:auto;padding:1rem;display:flex;flex-direction:column;gap:.5rem;min-height:120px}
...
function initGlobalChat () {
  ...
  panel.innerHTML = `
    <div class="gcw-head">
      <div class="gcw-head-shop">
        <img class="gcw-logo" id="gcw-logo" src="" alt="" hidden>
        <div>
          <span class="gcw-shop-name" id="gcw-shop-name"></span>
    ...
    <div class="gcw-tabs" id="gcw-tabs" hidden></div>
  ...
  const tabs      = document.getElementById('gcw-tabs')
  const logoEl    = document.getElementById('gcw-logo')
  const nameEl    = document.getElementById('gcw-shop-name')

  // ── Reiter aufbauen (nur bei mehreren Chats) ──
  if (sessions.length > 1) {
    tabs.hidden = false
    tabs.innerHTML = sessions.map(s => `
      <button class="gcw-tab${s.chat_id === activeSession.chat_id ? ' active' : ''}"
              data-chat-id="${esc(s.chat_id)}" type="button">
        ${s.shop_logo ? `<img src="${esc(s.shop_logo)}" alt="">` : ''}
        ${esc(s.shop_name || 'Chat')}
      </button>`).join('')

    tabs.querySelectorAll('.gcw-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        const s = sessions.find(x => x.chat_id === tab.dataset.chatId)
        if (!s || s.chat_id === activeSession.chat_id) return
        activeSession = s
        updateHeader()
        tabs.querySelectorAll('.gcw-tab').forEach(t =>
          t.classList.toggle('active', t.dataset.chatId === activeSession.chat_id))
        stopPolling()
        loadMessages()
        startPolling()
      })
  ...
  function updateHeader () {
    nameEl.textContent = activeSession.shop_name || 'Chat'
    if (activeSession.shop_logo) { logoEl.src = activeSession.shop_logo; logoEl.hidden = false }
  ...
  async function loadMessages () {
    try {
      const { data } = await supabase
        ...
        .eq('chat_id', activeSession.chat_id)
        .order('erstellt_am', { ascending: true })
      const msgs = data || []
      renderMessages(msgs)
      const hc = msgs.filter(m => m.von_haendler).length
      if (!isOpen && hc > lastHaendlerCount) {
        badge.hidden = false
        badge.textContent = hc - lastHaendlerCount
      }
  ...
  async function sendMessage () {
    ...
    try {
      const { error } = await supabase.from('chat_nachrichten')
        .insert({ chat_id: activeSession.chat_id, text, von_haendler: false })
      if (error) throw error
      await supabase.from('chats')
        .update({ aktualisiert_am: new Date().toISOString() })
        .eq('id', activeSession.chat_id)
      input.value = ''
